# Good Boy Records v9.0 — top pull-down pages + workflow fidelity

Built from the full v8.2 workflow-rendering source.

This release:
- moves the Markdown page tabs from the right edge into a horizontal equipment rail beneath the main fascia;
- pulls the selected page down from the top as an overlay without interrupting audio;
- supports the planned ~10 pages with horizontal overflow rather than wrapped tabs;
- changes the page drawer resizer from width to height;
- adds real nested Markdown-list rendering and a themed model-component tree on the Tech page;
- renders empty prompt-like ComfyUI widgets such as `caption` and `lyrics` as full multiline text areas;
- preserves the workflow's node positions, sizes, links, groups, colours and widget values;
- includes the six current page files supplied on 10 September 2026.
...
## Embedding Spotify from Markdown

A Spotify playlist can be embedded in any documentation or folder Markdown file with:

```md
[spotify: [https://open.spotify.com/playlist/XXXXXXXX](https://open.spotify.com/playlist/XXXXXXXX)]
```

The shorter form is also accepted:

```md
[spotify: https://open.spotify.com/playlist/XXXXXXXX]
```

The renderer converts the public Spotify URL to Spotify's read-only embed player.
Playlist, album, track, artist, show and episode URLs are accepted. Query strings such
as Spotify's `?si=...` sharing parameter are ignored when the embed URL is built.

## Embedding a ComfyUI workflow in a folder tab

Place a normal ComfyUI UI/workflow JSON file in `assets/workflows/`, then add this line to any `content-source/folders/*.md` file:

```text
[comfy-workflow: LLMExplanation.json]
```

Optional controls:

```text
[comfy-workflow: LLMExplanation.json height=650 focus=3404]
```

`height` sets the embedded viewer height in pixels (320–1100). `focus` opens centred on a specific node id; omit it to fit the complete workflow. The viewer is read-only and uses no ComfyUI runtime or backend.


## v7.5 drawer interaction patch

- Folder drawer resize handle is now a visible full-height rail with a larger grip.
- Drawer resizing tracks pointer movement globally, so dragging continues across the scrim/workflow.
- Folder tabs, close button and Comfy workflow controls have larger hit targets and stronger contrast.
- Mobile keeps the drawer full-width; horizontal resizing remains a desktop/tablet-wide interaction.

## Workflow media assets

The ComfyUI workflow viewer can render media loader/preview nodes without a
ComfyUI server. Keep the editable/source files here:

```text
content-source/
├── images/       # .png/.jpg/.webp/.gif plus .mp4/.webm/etc.
└── otheraudio/   # .mp3/.flac/.wav/.ogg/.m4a/.opus/etc.
```

Keep the filename selected in ComfyUI exactly the same as the file placed in
these folders. On every normal `BUILD-SHOWCASE` run, `build_workflow_media.py`
parses every `assets/workflows/*.json` file, checks every referenced media file
(including letter case), and rebuilds:

```text
assets/workflow-media/
├── images/
└── audio/
```

Those generated assets are served by the local preview and are already inside
`assets/`, so the existing GitHub Pages staging step publishes them too.
Missing or case-mismatched workflow media is a build error rather than a silent
broken preview on the live site.

Supported viewer media includes images, animated GIFs, MP4/WebM video and the
common browser audio formats. Video and audio are rendered with native browser
controls; GIFs animate as ordinary images.

## v8.0 workflow node layout fix

The read-only ComfyUI viewer now uses box-sized, compact node chrome matching the saved ComfyUI geometry more closely. Multiline text and dense widget stacks stay within their node bounds, and link endpoints match the revised port rows.

## v8.2 workflow viewer
Hybrid near/far workflow scaling and a simplified workflow toolbar. See assets/js/comfy-workflow.js for implementation.
