...
check("workflow media source folders are build managed",
      (ROOT / "tools/build_workflow_media.py").exists()
      and "build_workflow_media.sync(report)" in (ROOT / "tools/build_catalogue.py").read_text(encoding="utf-8"))
check("workflow viewer renders image video and audio media",
      "cw-media--" in (ROOT / "assets/js/comfy-workflow.js").read_text(encoding="utf-8")
      and "document.createElement(\"video\")" in (ROOT / "assets/js/comfy-workflow.js").read_text(encoding="utf-8")
